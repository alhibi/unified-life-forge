...
export default function PrayerTimes() {
  ...
  // `localStorage.lastLocation` synchronously, so on warm visits the very
  // first render already has coords. On cold visits Index.tsx fires the
  // permission prompt; we just react to whatever the hook hands us.
  // Cross-tab updates and `locationUpdated` events are handled inside the
  ...
  useEffect(() => {
    ...
    // elsewhere). The hook coalesces concurrent requests into one.
    if (locationStatus === 'idle') {
      void requestLocation();
      return;
    }

    // While the request is in flight don't pre-fetch with a stale value;
    // wait for the resolution. PrayerTimes already shows a skeleton.
  ...
  if (error) {
    return (
      <div className="rounded-3xl bg-destructive/10 border border-destructive/20 p-4 text-destructive text-center text-meta">
        {error}
      </div>
    );
  }
  ...
  return (
    <>
      ...
      >
        ...
        <span
          className={`text-meta flex-1 truncate ${
        ...
        {isNext && (
          <span className="text-micro font-bold tracking-[0.1em] text-primary me-2 shrink-0">
            {t('prayer.next.short')}
          </span>
        )}
        ...
        {language !== 'ar' && (
          <span className="text-meta text-foreground/50 shrink-0 me-2">{prayer.ar}</span>
        )}

        {/* Time */}
        <span
          className="text-mini font-medium text-foreground/70 tabular-nums min-w-[60px] text-end shrink-0"
          dir="ltr"
        >
          {prayer.time}
        </span>
    ...
    <div>
      ...
      <div className="flex items-center justify-between px-4 pt-2.5 pb-1.5">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-micro font-semibold text-muted-foreground" dir="rtl">
        ...
        {occasions.map((occ) => {
          ...
          return (
            ...
            >
              ...
              {/* Name */}
              <p className="text-micro font-semibold text-foreground leading-snug line-clamp-1 w-full">
                {occ.name}
              </p>
            </button>
          );
