...
export const INTERFACE_PROFILES_VERSION = 1 as const;
...
export function parseInterfaceProfilesImport(raw: string): SavedInterfaceProfile[] | null {
  ...
  try {
    ...
    if (
      !isRecord(parsed) ||
      parsed.version !== INTERFACE_PROFILES_VERSION ||
      !Array.isArray(parsed.profiles) ||
      parsed.profiles.length === 0
    ) {
      return null;
    }
    const requiredSettings: readonly (keyof InterfaceProfileSettings)[] = [
      'cornerSoftness',
      'density',
      'width',
      'border',
    ...
    const hasCompleteProfiles = parsed.profiles.every((profile) => {
      ...
      return requiredSettings.every((key) => Object.prototype.hasOwnProperty.call(settings, key));
    });
