...
export const useAuthStore = create<AuthState & AuthActions>()(
  persist((set) => ({ ...initialAuthState, login: (user, token) => set({ isAuthenticated: true, user, token, lastLogin: new Date().toISOString(), isInitializing: false }), logout: () => { try { Object.keys(localStorage).forEach(key => { if (key.includes('draft') || key.includes('cache')) localStorage.removeItem(key); }); } catch(e) { /* noop */ } set({ ...initialAuthState, isInitializing: false }); }, updateProfile: (profileUpdates) => set((state) => { if (!state.user) return state; return { user: { ...state.user, ...profileUpdates, updated_at: new Date().toISOString() } }; }), setInitializing: (isInitializing) => set({ isInitializing }) }),
