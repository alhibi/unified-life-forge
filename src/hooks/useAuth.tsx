...
async function signUp(username: string, password: string) {
  ...
  if (!isSupabaseConfigured) {
    ...
    // Supabase would have used so listeners update in lock-step. We
    // *await* this rather than firing it as `void` so the caller's
    // post-success navigation cannot race ahead of the state update
    // and momentarily render the auth-required pages with `user=null`.
    await syncAuthState(result.data?.session ?? null);
