...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const tokens = md3Mode
      ? generateMD3Tokens(isDark, isDark && blackMode)
      : generateThemeTokens(preset, paletteStyle as ThemeStyle, isDark, isDark && blackMode);
    ...
    if (theme === 'system') {
      ...
      const handler = (e: MediaQueryListEvent) => {
        ...
        const t2 = md3Mode
          ? generateMD3Tokens(e.matches, e.matches && blackMode)
          : generateThemeTokens(preset!, paletteStyle as ThemeStyle, e.matches, e.matches && blackMode);
        applyThemeTokens(t2);
      };
