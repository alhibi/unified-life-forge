...
function themeClassForCode(code: number, isDay: boolean): string {
  if (code <= 1) return isDay ? 'state-clear-day' : 'state-clear-night';
  if (code === 2 || code === 3) return isDay ? 'state-cloudy-day' : 'state-cloudy-night';
  if ((code >= 51 && code <= 57) || (code >= 61 && code <= 67) || (code >= 80 && code <= 82))
    return 'state-rainy';
  if ((code >= 71 && code <= 77) || (code >= 85 && code <= 86)) return 'state-snowy';
  if (code >= 95) return 'state-thunder';
  if (code === 45 || code === 48) return 'state-foggy';
  return isDay ? 'state-clear-day' : 'state-clear-night';
}

function Panel({ title, sub, children }: { title?: string; sub?: string; children: ReactNode }) {
  return (
    <section className="relative overflow-hidden material-card p-5">
      {(title || sub) && (
        <header className="pt-1 pb-4 flex items-end justify-between gap-3 border-b border-white/10 mb-4">
          {title && (
            <h2 className="font-montserrat font-bold text-[18px] leading-none text-white/95">
              {title}
            </h2>
          )}
          {sub && (
            <span className="text-[11px] tracking-[0.15em] uppercase text-white/80 font-bold tabular-nums text-end">
              {sub}
            </span>
          )}
        </header>
...
function Metric({
  ...
  return (
    <div className="min-w-0">
      <div className="flex items-center gap-1.5 text-white/80 font-bold min-w-0">
        {icon && <span className="[&>svg]:w-3.5 [&>svg]:h-3.5 text-white/90 shrink-0">{icon}</span>}
      ...
      <div className="mt-1 flex items-baseline gap-1 tabular-nums" dir="ltr">
        <span className="font-montserrat font-bold text-[22px] leading-none text-white">
          {value}
        </span>
        {unit && <span className="text-[12px] text-white/80 font-bold">{unit}</span>}
      </div>
      {hint && <p className="mt-0.5 text-[11px] text-white/70 font-medium truncate">{hint}</p>}
    </div>
  );
}
...
function GaugeTile({
  ...
  return (
    <div className="material-card p-5 min-w-0 h-full flex flex-col justify-between">
      <div className="flex items-center justify-between gap-2 text-white/80 font-bold">
        <span className="text-[11px] tracking-[0.12em] uppercase truncate">{label}</span>
        {icon && <span className="[&>svg]:w-4 [&>svg]:h-4 text-white shrink-0">{icon}</span>}
      </div>
      <div className="mt-3 flex items-center gap-3">
        ...
        <div className="min-w-0">
          <div className="flex items-baseline gap-1 tabular-nums" dir="ltr">
            <span className="font-montserrat font-bold text-[24px] leading-none text-white">
              {value}
            </span>
            {unit && <span className="text-[11px] text-white/80 font-bold">{unit}</span>}
          </div>
          {hint && <p className="mt-1 text-[11px] text-white/70 font-medium truncate">{hint}</p>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-white bg-white/5 p-3 rounded-2xl border border-white/10">
          ...
          <button
            ...
            className="w-8 h-8 flex items-center justify-center rounded-xl border border-white/20 bg-white/10 text-white shrink-0 transition-transform active:scale-95 ms-2"
          >
            <ChevronDown
          ...
          {isExpanded && (
            ...
            >
              {rows.map((r) => (
                <div
                  key={r.id}
                  className="grid grid-cols-[1fr_auto_auto] items-center gap-2 text-[11px] border-b border-white/10 pb-2 last:border-0 last:pb-0"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span
                      className={`w-2 h-2 rounded-full shrink-0 ${r.state === 'closed' ? 'bg-green-400' : r.state === 'half_open' ? 'bg-yellow-400' : 'bg-red-400'}`}
                    />
                    <span className="truncate text-white font-medium">{r.label}</span>
                  </div>
                  <span className="text-white/80 tabular-nums">
                    وزن {r.effectiveWeight.toFixed(2)}
                  </span>
                  <span className="text-white/80 tabular-nums">{r.avgResponseMs}ms</span>
                </div>
              ))}
            </motion.div>
          )}
...
function HourlyRibbon({
  ...
  return (
    <Panel title={ar ? 'الساعات القادمة' : 'Kommende Stunden'} sub={ar ? '12 ساعة' : '12 Stunden'}>
      <div className="-mx-5 px-5 overflow-x-auto no-scrollbar" dir="ltr">
          ...
          {slice.map((e, i) => {
            ...
            return (
              <motion.div
                ...
                className="w-[62px] shrink-0 rounded-[22px] border border-white/10 bg-white/5 px-2 py-3.5 text-center flex flex-col justify-between"
              >
                <div className="text-[10px] tracking-[0.1em] uppercase text-white/90 font-bold tabular-nums">
                  {i === 0
                    ? ar
                      ? 'الآن'
                      : 'jetzt'
                ...
                <Icon className="w-5 h-5 mx-auto my-2 text-white" strokeWidth={1.4} />
                <div className="font-montserrat font-bold text-[19px] leading-none text-white tabular-nums">
                  {Math.round(e.temperature_c)}°
                </div>
                <div className="mt-2 h-8 rounded-full bg-white/10 relative overflow-hidden">
                  <div
                    className="absolute inset-x-0 bottom-0 rounded-full bg-gradient-to-t from-white to-white/30"
                    style={{ height: `${Math.max(10, heat * 100)}%` }}
                  />
                </div>
                <div className="mt-1.5 text-[9px] text-white/95 font-bold tabular-nums">
...
function WindCompass({
  ...
  return (
    <Panel
      title={ar ? 'الرياح وحركتها الجوية' : 'Wind & Dynamik'}
      sub={ar ? 'بوصلة حية' : 'Kompass'}
    >
      <div className="flex items-center gap-5">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline gap-1 tabular-nums" dir="ltr">
            <span className="font-montserrat font-extrabold text-[36px] leading-none text-white">
              {Math.round(speed)}
            </span>
            <span className="text-[12px] text-white/90 font-bold">km/h</span>
          </div>
          <p className="mt-1 text-[11px] text-white/80 font-semibold">
            {cardinal} · {Math.round(dirDeg)}°
          </p>
          <p className="mt-2 text-[11px] text-white/90 leading-snug font-semibold">{beaufort}</p>
          <p className="mt-1 text-[10px] text-white/80 tabular-nums font-semibold" dir="ltr">
            {ar ? 'هبات' : 'Böen'} {Math.round(gusts)} km/h
          </p>
        </div>
      </div>
    </Panel>
...
function AQIGauge({
  ...
  const bands = [
    { to: 25, color: '#4ade80', label: ar ? 'ممتاز' : 'sehr gut' },
    { to: 50, color: '#a3e635', label: ar ? 'جيد' : 'gut' },
    { to: 75, color: '#facc15', label: ar ? 'متوسط' : 'mäßig' },
    { to: 100, color: '#f97316', label: ar ? 'ضعيف' : 'schlecht' },
    { to: 150, color: '#ef4444', label: ar ? 'رديء جداً' : 'sehr schlecht' },
  ];
  ...
  return (
    <Panel
      title={ar ? 'جودة الهواء والغبار' : 'Luftqualität'}
      sub={source ?? (ar ? 'نموذج' : 'Modell')}
    >
      <div className="flex items-end gap-5" dir="ltr">
        <div className="relative shrink-0">
          ...
          <div className="absolute inset-x-0 -bottom-1 text-center">
            <div className="font-montserrat font-bold text-[26px] leading-none text-white tabular-nums">
              {Math.round(caqi)}
            </div>
            <div
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[12px] leading-relaxed text-white/90 font-medium line-clamp-3">
            {advisory}
          </p>
          <div className="mt-2 flex items-center gap-2 text-[10px] tracking-[0.16em] uppercase text-white/80">
            <span>{ar ? 'مؤشر الصحة البشري' : 'Gesundheitsindex'}</span>
            <span className="tabular-nums text-white font-bold">{healthScore}/100</span>
          </div>
        </div>
      </div>
      <div className="mt-5 grid grid-cols-3 gap-2.5">
        {pollutants.map((p) => {
          ...
          const color = ratio < 0.5 ? '#4ade80' : ratio < 1 ? '#facc15' : '#ef4444';
          return (
            <div
              key={p.label}
              className="rounded-2xl border border-white/10 bg-white/5 px-3 py-2.5 min-w-0"
            >
              <div className="flex items-center justify-between text-[9px] tracking-[0.12em] uppercase text-white/80 font-bold">
                <span>{p.label}</span>
                <span className="tabular-nums font-extrabold" style={{ color }}>
                  {p.value.toFixed(p.label === 'CO' ? 2 : 1)}
                  <span className="text-white/60 ms-0.5">{p.unit}</span>
                </span>
              </div>
...
function LiveSunArc({
  ...
  return (
    <Panel
      title={ar ? 'مسار الشمس والقبة السماوية' : 'Sonnenlauf & Himmel'}
      sub={ar ? `${dayLengthH.toFixed(1)} ساعة` : `${dayLengthH.toFixed(1)} h`}
    >
      <div className="relative" style={{ aspectRatio: `${W} / ${H}` }} dir="ltr">
        ...
        >
          ...
          >
            <circle cx={cx} cy={cy} r="14" fill="#ffffff" fillOpacity="0.25" />
            <circle cx={cx} cy={cy} r="6" fill="#ffffff" />
          </motion.g>
        </svg>
      </div>
      <div className="mt-3 grid grid-cols-3 gap-2 text-center" dir="ltr">
        <div>
          <div className="flex items-center justify-center gap-1 text-[11px] tracking-[0.12em] uppercase text-white/90 font-bold">
            <Sunrise className="w-3.5 h-3.5 text-white" /> {ar ? 'شروق' : 'Aufgang'}
          </div>
          <div className="mt-1 font-montserrat font-bold text-[18px] leading-none text-white tabular-nums">
            {timeLabel(sunrise, locale)}
          </div>
        </div>
        <div>
          <div className="flex items-center justify-center gap-1 text-[11px] tracking-[0.12em] uppercase text-white/90 font-bold">
            <Sun className="w-3.5 h-3.5 text-white" /> {ar ? 'الآن' : 'Höhe'}
          </div>
          <div className="mt-1 font-montserrat font-bold text-[18px] leading-none text-white tabular-nums">
            {elevationDeg.toFixed(0)}°
          </div>
          <div className="text-[11px] text-white/90 font-bold tabular-nums">
            {Math.round(azimuthDeg)}°
          </div>
        </div>
        <div>
          <div className="flex items-center justify-center gap-1 text-[11px] tracking-[0.12em] uppercase text-white/90 font-bold">
            <Sunset className="w-3.5 h-3.5 text-white" /> {ar ? 'غروب' : 'Untergang'}
          </div>
          <div className="mt-1 font-montserrat font-bold text-[18px] leading-none text-white tabular-nums">
...
function DailyRangeStrip({
  ...
  return (
    <Panel
      title={ar ? 'الأيام القادمة وحركة الحرارة' : 'Nächste Tage'}
      sub={`${days.length} ${ar ? 'أيام' : 'Tage'}`}
    >
        ...
        {days.map((d, i) => {
          ...
          return (
            <motion.div
              ...
              className="grid grid-cols-[56px_28px_1fr_auto] items-center gap-4"
            >
              ...
              <DayIcon className="w-4.5 h-4.5 text-white" strokeWidth={1.4} />
              <div className="relative h-2 rounded-full bg-white/10 overflow-hidden">
                <div
                  className="absolute inset-y-0 rounded-full bg-gradient-to-r from-sky-300 via-white to-amber-300"
                  style={{ left: `${leftPct}%`, right: `${100 - rightPct}%` }}
                />
              </div>
              <span className="text-[12px] tabular-nums text-white font-bold">
                <span className="text-white/70">{Math.round(d.low_c)}°</span>
...
export default function Weather() {
  ...
  const { snapshot, status, tier, isRefreshing, refresh } = useWeather(
    ar ? 'ar' : 'de',
    selectedCoords,
  );
  ...
  const currentThemeClass = useMemo(() => {
    return themeClassForCode(currentHour?.weather_code ?? 0, currentHour?.is_day ?? true);
  }, [currentHour]);

  if (status === 'loading' && !snapshot) {
    return (
      <div className="min-h-screen p-6 grid place-items-center bg-slate-900 text-white">
        <span className="font-montserrat font-semibold text-xl text-white/85 animate-pulse">
          {ar ? 'نقرأ الغلاف الجوي ونجمع الأرصاد…' : 'Der Himmel wird gelesen…'}
        </span>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen p-6 grid place-items-center bg-slate-900 text-white">
        <span className="text-sm text-white/70">
          {ar ? 'تعذر تحميل بيانات الطقس.' : 'Wetterdaten nicht verfügbar.'}
        </span>
      </div>
    );
  }
  ...
  return (
    <div
      dir={ar ? 'rtl' : 'ltr'}
      className={`weather-theme ${currentThemeClass} min-h-screen pb-24`}
    >
      <Helmet>
        <title>
          {ar ? 'لوحة الأرصاد والطقس المتكاملة — SmartHub' : 'SmartHub Wetter — Messpanel'}
        </title>
        <meta
          name="description"
          content={
            ar
              ...
              : 'Entdecken Sie das Wetter mit Partikelsimulatoren, thermischen Rechnern und Fitnessplanern.'
          }
        />
      </Helmet>

      {/* Sticky App Header - Michael Kokoschka App style */}
      <div className="sticky top-0 z-40 border-b border-white/10 bg-black/10 backdrop-blur-xl">
        <div className="px-5 py-4 flex items-center gap-3">
          <BackButton />
          <div className="flex-1 min-w-0 text-center">
            <h1 className="font-montserrat font-bold text-[22px] leading-none text-white truncate">
              {selectedCoords?.name ||
                (ar ? 'لوحة الأرصاد الجوية والفيزياء' : 'Wetter-Instrumente')}
            </h1>
            <p
              className="mt-2 text-[11px] tracking-[0.15em] uppercase text-white/90 font-bold tabular-nums"
              dir="ltr"
            >
          ...
          <button
            onClick={refresh}
            aria-label={ar ? 'تحديث الطقس' : 'Wetter aktualisieren'}
            className="w-10 h-10 rounded-2xl border border-white/20 bg-white/10 flex items-center justify-center active:scale-95 transition-transform"
          >
      ...
      {/* Main Container */}
      <main className="px-5 pt-5 space-y-6 max-w-4xl mx-auto">
        {/* City Search Module */}
        <CitySearch onSelectCity={handleCitySelect} ar={ar} />

        {/* Dynamic Segmented Control main tabs - Pixel-like Rounded Shapes */}
        <div className="flex bg-white/10 border border-white/15 p-1.5 rounded-[24px] gap-1.5 sticky top-20 z-35 backdrop-blur-md shadow-lg">
          {mainTabs.map((t) => {
            ...
            return (
              <button
                ...
                className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-1.5 py-2.5 px-2 rounded-[18px] text-xs font-bold transition-all duration-200 active:scale-95 ${
                  active
        ...
        <AnimatePresence mode="wait">
          {activeMainTab === 'now' && (
            <motion.div
              ...
              className="space-y-6"
            >
              {/* Giant Weather Hero Section */}
              <section className="relative overflow-hidden material-card p-6">
                <div className="relative z-10">
                  <div className="flex items-start justify-between gap-5">
                    <div className="min-w-0">
                      <p className="text-[12px] tracking-[0.2em] uppercase text-white/90 font-extrabold">
                        {comfortLabel(snapshot.temperature.thermal_comfort_level, ar)}
                      </p>
                      <div className="mt-4 flex items-end gap-3" dir="ltr">
                        <span className="font-montserrat text-[84px] leading-[0.72] text-white tabular-nums drop-shadow-[0_4px_24px_rgba(255,255,255,0.15)] font-extrabold">
                          {Math.round(snapshot.temperature.actual_c)}°
                        </span>
                        <span className="mb-1 font-montserrat text-[26px] leading-none text-white/80 font-bold tabular-nums">
                          /{Math.round(snapshot.temperature.apparent_c)}°
                        </span>
                      </div>
                      <p className="mt-5 text-[15px] text-white font-extrabold">
                        {weatherLabel(currentHour?.weather_code ?? 0, ar)}
                      </p>
                      <p
                        className="mt-2 text-[12px] text-white/90 font-bold tabular-nums"
                        dir="ltr"
                      >
                        ↑ {Math.round(snapshot.temperature.daily_high_c)}° · ↓{' '}
                        {Math.round(snapshot.temperature.daily_low_c)}° · {ar ? 'ندى' : 'Taupunkt'}{' '}
                    ...
                    <div className="flex flex-col items-center gap-3 shrink-0">
                      ...
                      >
                        ...
                        <CurrentIcon className="relative w-22 h-22 text-white" strokeWidth={1.05} />
                      </motion.div>
                      <div className="text-center">
                        <div className="text-[11px] tracking-[0.15em] uppercase text-white/80 font-bold">
                          {ar ? 'ثقة التنبؤ' : 'Ensemble Vertrauen'}
                        </div>
                        <div
                          className="font-montserrat font-bold text-[22px] leading-none text-white tabular-nums"
                          dir="ltr"
                        >
                          {conf}%
                        </div>
                  ...
                  <div className="mt-6 h-2 rounded-full bg-white/15 overflow-hidden" dir="ltr">
                    <motion.div
                  ...
                  <div className="mt-5 grid grid-cols-4 gap-3 text-center" dir="ltr">
                    {[
                      {
                        label: ar ? 'ضغط' : 'Druck',
                      ...
                      {
                        label: ar ? 'ندى' : 'Taupunkt',
                        value: `${Math.round(snapshot.temperature.dew_point_c)}`,
                        unit: '°',
                      },
                      {
                        label: ar ? 'انزعاج' : 'Discomfort',
                        value: snapshot.temperature.discomfort_index.toFixed(1),
                        unit: '',
                      },
                      {
                        label: ar ? 'أقصى UV' : 'UV max',
                        value: snapshot.solar.uv_max_today.toFixed(1),
                        unit: '',
                      },
                    ].map((m) => (
                      <div
                        key={m.label}
                        className="rounded-2xl border border-white/10 bg-white/5 py-3 px-2 shadow-sm"
                      >
                        <div className="text-[11px] tracking-[0.1em] uppercase text-white/90 font-bold">
                          {m.label}
                        </div>
                        <div className="mt-1 flex items-baseline justify-center gap-0.5 tabular-nums">
                          <span className="font-montserrat font-bold text-[18px] leading-none text-white">
                            {m.value}
                          </span>
                          {m.unit && (
                            <span className="text-[10px] text-white/90 font-bold">{m.unit}</span>
                          )}
                        </div>
                      </div>
                    ))}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                />
                <GaugeTile
                  label={ar ? 'الرطوبة النسبية' : 'Rel. Feuchte'}
                  value={Math.round(snapshot.moisture.relative_humidity_percent)}
                  unit="%"
                  pctValue={snapshot.moisture.relative_humidity_percent / 100}
                  hint={`${ar ? 'ندى' : 'Taupunkt'} ${Math.round(snapshot.temperature.dew_point_c)}°`}
                  icon={<Droplets />}
                />
                <GaugeTile
                  label={ar ? 'تغطية الغيوم' : 'Bewölkung'}
                ...
                />
                <GaugeTile
                  label={ar ? 'مدى الرؤية الأفقية' : 'Sichtweite'}
          ...
          {activeMainTab === 'forecast' && (
            ...
            >
              ...
              <Panel
                title={ar ? 'المؤشرات الفيزيائية المشتقة' : 'Abgeleitete Werte'}
                sub={ar ? 'علم الغلاف' : 'Atmosphäre'}
              >
                <div className="grid grid-cols-2 gap-y-5 gap-x-5">
                  ...
                  />
                  <Metric
                    label={ar ? 'رطوبة مطلقة' : 'Abs. Feuchte'}
                  ...
                  />
                  <Metric
                    label={ar ? 'قاعدة السحب' : 'Wolkenbasis'}
          ...
          {activeMainTab === 'radar' && (
            ...
            >
              ...
              {/* Realtime Particle simulator & interactive radar */}
              <RadarMap
              ...
              <Panel
                title={ar ? 'القياسات الفيزيائية اللحظية الفريدة' : 'Einzigartige Messwerte jetzt'}
              ...
              >
                <div className="grid grid-cols-3 gap-y-5 gap-x-4">
                  ...
                  />
                  <Metric
                    label={ar ? 'احتمالية الضباب' : 'Nebel-Wkt.'}
          ...
          {activeMainTab === 'lab' && (
            <motion.div
              ...
              className="space-y-6"
            >
              {/* Smart Weather Planner & Medical Advisories */}
              <WeatherPlanner
              ...
              {/* Physics Meteorology Calculator simulator */}
              <MeteorologyConsole ar={ar} />

              {/* Astronomics and Lunar stats */}
              <Panel
                title={ar ? 'الفلك والضوء والشهور القمرية' : 'Astronomie & Mond'}
                sub={ar ? 'تفاصيل فلكية' : 'Details'}
              >
                <div className="grid grid-cols-3 gap-y-5 gap-x-4">
                  <Metric
                    label={ar ? 'طول النهار' : 'Tageslänge'}
                    value={snapshot.astronomical.day_length_hours.toFixed(1)}
                    unit="h"
                  />
                  <Metric
                    label={ar ? 'الشمس' : 'Sonnenhöhe'}
                  ...
                  />
                  <Metric
                    label={ar ? 'الساعة الذهبية' : 'Goldene Stunde'}
        ...
        <footer
          className="pt-4 pb-8 text-center text-[10px] tracking-[0.18em] uppercase text-white/80 tabular-nums"
          dir="ltr"
        >
