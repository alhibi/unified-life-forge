  ...
  function ReactionPill({ emoji, count, reactedByMe, onClick, ariaLabel }: ReactionPillProps) {
    return (
      <motion.button
        ...
        className={cn(
          'inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-mini border select-none',
          reactedByMe
      ...
      >
        ...
        {count > 1 && (
          <motion.span
            className="text-micro text-muted-foreground font-medium tabular-nums"
            key={count}
            initial={{ scale: 0, y: 4 }}
            animate={{ scale: 1, y: 0 }}
          >
...
export const MessageMetaRow = React.memo(function MessageMetaRow({
  ...
  return (
    <span
      className={cn(
        'inline-flex items-center gap-1 text-micro select-none shrink-0 mt-0.5',
        ...
        {shown.map((s, i) => (
          ...
          >
            {s.avatarUrl ? (
              ...
              <span className="w-full h-full flex items-center justify-center text-micro font-bold text-muted-foreground">
