        ...
        <div className="grid grid-cols-4 gap-3">
          {options.map((opt, i) => (
            ...
            >
              ...
              <span className="text-micro text-muted-foreground font-medium">{opt.label}</span>
            </motion.button>
          ))}
        </div>
      </div>
        ...
        {filtered.map((user, i) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-mini font-medium text-foreground truncate">
                {user.displayName || user.username}
              </p>
              <p className="text-micro text-muted-foreground">@{user.username}</p>
            </div>
          </motion.button>
        ))}
      </div>
...
const CharacterCounter = React.memo(function CharacterCounter({ count }: { count: number }) {
  ...
  return (
    <motion.span
      ...
      className={cn(
        'absolute top-1 end-2 text-micro font-mono tabular-nums',
        isOver
            ...
            <div className="mx-3 mt-2 rounded-xl bg-primary/8 border border-primary/15 overflow-hidden">
              <div className="flex items-start gap-2 p-2.5">
                <div className="flex-1 min-w-0 border-s-2 border-primary ps-2.5">
                  ...
                  <p className="text-micro text-muted-foreground truncate" dir="auto">
        ...
        {replyTo && !disableTextUI && !editingMessage && (
          ...
          >
            <div className="mx-3 mt-2 rounded-xl bg-accent/20 border border-border/15 overflow-hidden">
              <div className="flex items-start gap-2 p-2.5">
                <div className="flex-1 min-w-0 border-s-2 border-primary ps-2.5">
                  <span className="text-micro font-semibold text-primary block">
        ...
        {showPreviewBar ? (
          ...
          >
            ...
            <div className="flex-1 flex items-center gap-3 bg-muted/20 rounded-full px-4 h-10">
              ...
              />
              <span
                className="text-mini font-mono text-foreground tabular-nums tracking-wide"
                role="timer"
          ...
          >
            ...
            <div className="flex-1 relative flex items-end bg-muted/15 border border-border/15 rounded-3xl overflow-visible transition-all duration-200 focus-within:border-primary/25 focus-within:bg-muted/5 ">
              ...
              <Textarea
                ...
                dir="auto"
                className="flex-1 border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 min-h-[40px] max-h-[140px] resize-none ps-1.5 pe-3.5 py-[10px] text-meta leading-relaxed placeholder:text-muted-foreground/40"
              />
            </div>
