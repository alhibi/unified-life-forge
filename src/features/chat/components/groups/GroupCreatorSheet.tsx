...
const GroupCreatorSheet: React.FC<GroupCreatorSheetProps> = ({ isOpen, onClose, onCreated, initialKind = 'group',
 ...
 if (step === 'fill-meta' && kind === 'group') { setStep('pick-members'); return; }
          ...
          <h2 id="group-creator-title" className="text-body font-semibold flex-1 truncate">
            {step === 'pick-members'
              ? ('إضافة أعضاء')
              : kind === 'group'
                ? ('مجموعة جديدة')
 ...
 {'اختياري'}
            ...
            {/* Title */}
            <div>
              <label className="text-micro font-medium text-muted-foreground mb-1.5 block">
                {'الاسم'}
              ...
              <input
                ...
                className="w-full bg-muted/30 rounded-2xl px-4 h-11 text-meta outline-none focus:ring-2 focus:ring-primary/30"
                autoFocus
                dir="auto"
              />
            ...
            {/* Permissions hint */}
            <div className="rounded-2xl bg-muted/15 border border-border/15 p-3.5 text-mini text-muted-foreground leading-relaxed">
              {kind === 'group'
