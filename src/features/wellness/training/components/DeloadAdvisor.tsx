    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-micro uppercase tracking-wider font-semibold" style={{ color }}>
              {T.ready[lang]}
            </p>
            <p className="text-meta font-bold leading-tight text-foreground">
      ...
      <div className="space-y-1">
        ...
        <div className="flex justify-between text-micro text-muted-foreground/70 tabular-nums" dir="ltr">
          <span>0.0</span>
          <span>0.8</span>
          <span>1.3</span>
          <span>1.5</span>
...
function Metric({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="bg-muted/30 rounded-lg p-1.5 text-center">
      ...
      <p className="text-meta font-bold tabular-nums" style={{ color: color ?? 'currentColor' }}>{value}</p>
    </div>
  );
}
