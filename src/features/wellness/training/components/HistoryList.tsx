  ...
  if (workouts.length === 0) {
    return (
      <div className={`bg-card border border-border/40 rounded-2xl p-6 text-center ${className}`}>
        ...
        <p className="text-[12px] text-muted-foreground">{T.empty[lang]}</p>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-card border border-border/40 rounded-xl ps-8 pe-3 py-2 text-[12px] text-foreground outline-none focus:border-primary/50"
          />
        </div>
      </div>
    ...
    >
      ...
      >
        <div className="min-w-0 flex-1">
          ...
          {session.title && (
            <p className="text-[13px] font-bold text-foreground mt-0.5 truncate">{session.title}</p>
          )}
              ...
              {session.exercises.map((ex, i) => {
                ...
                return (
                  <div key={i} className="bg-muted/30 rounded-lg p-2 space-y-1">
                    ...
                    <ul className="text-[10px] text-muted-foreground tabular-nums space-y-0.5" dir="ltr">
                      {ex.sets.map((s, j) => (
                        <li key={j}>
