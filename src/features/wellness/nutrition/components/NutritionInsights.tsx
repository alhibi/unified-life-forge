...
import {
  ...
  lowestGIFoods, foodsHighInVitamin, foodsHighInMineral,
  TOTAL_FOOD_COUNT,
} from '../index';
...
const T = {
  ...
  topProtein: { ar: 'أفضل مصادر البروتين (نسبة للسعرات)', de: 'Beste Proteinquellen (pro Kalorie)' },
...
export default function NutritionInsights() {
  ...
  return (
    <div className="space-y-5 pb-10">
      {/* Header */}
      <div className="flex items-center gap-2 bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-xl p-3 border border-purple-500/20">
        <Sparkles className="w-5 h-5 text-purple-500" />
        <div>
          <p className="text-[11px] text-muted-foreground">{T.title[lang]}</p>
          <p className="text-xs font-bold text-foreground">
            {lang === 'ar' ? `تحليل ${TOTAL_FOOD_COUNT} عنصر غذائي` : `Analyse von ${TOTAL_FOOD_COUNT} Lebensmitteln`}
          </p>
        </div>
      </div>
          ...
          <div className="space-y-1">
            {foods.map((food, i) => (
              ...
              >
                ...
                <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md" style={{ backgroundColor: `${color}10`, color }}>
                  {metric(food)}
                </span>
              </div>
            ))}
