...
export default function FoodCard({ food, lang, onClick, compact }: Props) {
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      whileTap={{ scale: 0.97 }}
      className="w-full flex items-center gap-3 p-3 rounded-xl bg-card border border-border/40 hover:border-primary/30 transition-motion text-start relative"
    >
      {/* Emoji icon */}
      <div
        ...
        {!compact && (
          <div className="flex items-center gap-3 mt-1 text-micro text-muted-foreground" dir="ltr">
            <span className="inline-flex items-center gap-0.5">
              <Flame className="w-2.5 h-2.5 text-signal" />
              <span className="font-semibold text-foreground">{n.kcal}</span>
              <span>kcal</span>
            </span>
            <span className="text-data-5 font-medium">P:{n.protein}g</span>
            <span className="text-signal">C:{n.carbs}g</span>
            <span className="text-data-4">F:{n.fat}g</span>
          </div>
        )}
      </div>

      {/* Favorite indicator */}
      {fav && <Heart className="w-3 h-3 text-destructive fill-destructive shrink-0" />}
