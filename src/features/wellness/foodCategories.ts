...
export const CATEGORY_META: Record<FoodCategory, CategoryMeta> = {
  fruit:        { icon: Apple,    color: 'text-destructive',     bg: 'bg-destructive/10',     order: 1,  label: { ar: 'فواكه',              } },
  citrus:       { icon: Citrus,   color: 'text-signal',  bg: 'bg-signal/10',  order: 2,  label: { ar: 'حمضيات',     } },
  berry:        { icon: Cherry,   color: 'text-data-5',    bg: 'bg-data-5/10',    order: 3,  label: { ar: 'توتيات',            } },
  vegetable:    { icon: Carrot,   color: 'text-signal',  bg: 'bg-signal/10',  order: 4,  label: { ar: 'خضروات',            } },
  leafy:        { icon: Leaf,     color: 'text-data-1', bg: 'bg-data-1/10', order: 5,  label: { ar: 'ورقيات خضراء',       } },
  starchy_veg:  { icon: Salad,    color: 'text-signal',   bg: 'bg-signal/10',   order: 6,  label: { ar: 'خضروات نشوية',      } },
  meat:         { icon: Beef,     color: 'text-data-5',    bg: 'bg-data-5/10',    order: 7,  label: { ar: 'لحوم ودواجن',} },
  seafood:      { icon: Fish,     color: 'text-data-4',     bg: 'bg-data-4/10',     order: 8,  label: { ar: 'مأكولات بحرية',     } },
  egg:          { icon: Egg,      color: 'text-signal',   bg: 'bg-signal/10',   order: 9,  label: { ar: 'بيض',              } },
  dairy:        { icon: Milk,     color: 'text-data-4',    bg: 'bg-data-4/10',    order: 10, label: { ar: 'ألبان',     } },
  legume:       { icon: Bean,     color: 'text-data-6',  bg: 'bg-data-6/10',  order: 11, label: { ar: 'بقوليات',     } },
  nut_seed:     { icon: Nut,      color: 'text-signal',   bg: 'bg-signal/10',   order: 12, label: { ar: 'مكسرات وبذور',     } },
  grain:        { icon: Wheat,    color: 'text-signal',  bg: 'bg-signal/10',  order: 13, label: { ar: 'حبوب ونشويات',          } },
  beverage:     { icon: CupSoda,  color: 'text-data-4',    bg: 'bg-data-4/10',    order: 14, label: { ar: 'مشروبات',          } },
  water:        { icon: Droplets, color: 'text-data-4',    bg: 'bg-data-4/10',    order: 15, label: { ar: 'ماء',            } },
  fat_oil:      { icon: Droplet,  color: 'text-signal',  bg: 'bg-signal/10',  order: 16, label: { ar: 'دهون وزيوت',       } },
  spice_herb:   { icon: Sprout,   color: 'text-data-1', bg: 'bg-data-1/10', order: 17, label: { ar: 'توابل وأعشاب', } },
  sweet_treat:  { icon: Cookie,   color: 'text-signal',   bg: 'bg-signal/10',   order: 18, label: { ar: 'حلويات',             } },
};
