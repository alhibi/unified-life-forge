...
export const CATEGORY_META: Record<FoodCategory, CategoryMeta> = {
  fruit:        { icon: Apple,    color: 'text-red-500',     bg: 'bg-red-500/10',     order: 1,  label: { ar: 'فواكه',              } },
  citrus:       { icon: Citrus,   color: 'text-orange-500',  bg: 'bg-orange-500/10',  order: 2,  label: { ar: 'حمضيات',     } },
  berry:        { icon: Cherry,   color: 'text-pink-500',    bg: 'bg-pink-500/10',    order: 3,  label: { ar: 'توتيات',            } },
  vegetable:    { icon: Carrot,   color: 'text-orange-400',  bg: 'bg-orange-400/10',  order: 4,  label: { ar: 'خضروات',            } },
  leafy:        { icon: Leaf,     color: 'text-emerald-500', bg: 'bg-emerald-500/10', order: 5,  label: { ar: 'ورقيات خضراء',       } },
  starchy_veg:  { icon: Salad,    color: 'text-amber-600',   bg: 'bg-amber-600/10',   order: 6,  label: { ar: 'خضروات نشوية',      } },
  meat:         { icon: Beef,     color: 'text-rose-500',    bg: 'bg-rose-500/10',    order: 7,  label: { ar: 'لحوم ودواجن',} },
  seafood:      { icon: Fish,     color: 'text-sky-500',     bg: 'bg-sky-500/10',     order: 8,  label: { ar: 'مأكولات بحرية',     } },
  egg:          { icon: Egg,      color: 'text-amber-400',   bg: 'bg-amber-400/10',   order: 9,  label: { ar: 'بيض',              } },
  dairy:        { icon: Milk,     color: 'text-blue-300',    bg: 'bg-blue-300/10',    order: 10, label: { ar: 'ألبان',     } },
  legume:       { icon: Bean,     color: 'text-purple-500',  bg: 'bg-purple-500/10',  order: 11, label: { ar: 'بقوليات',     } },
  nut_seed:     { icon: Nut,      color: 'text-amber-700',   bg: 'bg-amber-700/10',   order: 12, label: { ar: 'مكسرات وبذور',     } },
  grain:        { icon: Wheat,    color: 'text-yellow-500',  bg: 'bg-yellow-500/10',  order: 13, label: { ar: 'حبوب ونشويات',          } },
  beverage:     { icon: CupSoda,  color: 'text-cyan-500',    bg: 'bg-cyan-500/10',    order: 14, label: { ar: 'مشروبات',          } },
  water:        { icon: Droplets, color: 'text-cyan-400',    bg: 'bg-cyan-400/10',    order: 15, label: { ar: 'ماء',            } },
  fat_oil:      { icon: Droplet,  color: 'text-yellow-600',  bg: 'bg-yellow-600/10',  order: 16, label: { ar: 'دهون وزيوت',       } },
  spice_herb:   { icon: Sprout,   color: 'text-emerald-600', bg: 'bg-emerald-600/10', order: 17, label: { ar: 'توابل وأعشاب', } },
  sweet_treat:  { icon: Cookie,   color: 'text-amber-500',   bg: 'bg-amber-500/10',   order: 18, label: { ar: 'حلويات',             } },
};
