...
import { Plus, Search as SearchIcon, BookOpen, Trash2, Loader2 } from '@/lib/icons';
...
export default function ArchiveHome() {
  ...
  useEffect(() => {
    ...
    archiveApi.list()
      ...
      .catch(err => { console.error(err); if (alive) setItems([]); })
  ...
  return (
    <PageShell>
      <SEO title="الأرشيف المعرفي — SmartHub" description="أرشيفك الشخصي من المونوغرافات المولّدة بذكاء اصطناعي: موضوع، عمق، وقراءة نظيفة." path="/archive" />
      ...
      <p className="text-[13px] text-muted-foreground leading-relaxed mb-1">
        اقترح موضوعاً ومستوى عمق، وسيبني المحرك مخطّطاً هرمياً ثم يكتب كل قسم فرعي على حدة، ثم يحفظه هنا برقم أرشيفي متسلسل.
      </p>
      ...
      {!user ? (
        <AppCard className="text-center py-10">
          ...
          <button onClick={() => navigate('/auth')} className="rounded-full bg-primary text-primary-foreground px-4 py-2 text-sm font-semibold">
            تسجيل الدخول
          </button>
        </AppCard>
      ) : loading ? (
        ...
        <AppCard className="text-center py-12">
          ...
          {items && items.length === 0 && (
            <button onClick={() => navigate('/archive/new')} className="rounded-full bg-primary text-primary-foreground px-4 py-2 text-sm font-semibold">
              توليد أول مستند
            </button>
          )}
        </AppCard>
        ...
        <div className="flex flex-col gap-3">
          <AnimatePresence initial={false}>
            {filtered.map((d, i) => (
              ...
              >
                <AppCard pressable onClick={() => navigate(`/archive/${d.id}`)} className="cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] text-muted-foreground">
                          {readingTime(d.word_count)} د · {d.word_count.toLocaleString('ar-EG')} كلمة
                        </span>
                      </div>
                      <h3 className="font-bold text-foreground leading-snug mb-1 line-clamp-2">{d.title}</h3>
                      {d.abstract && (
