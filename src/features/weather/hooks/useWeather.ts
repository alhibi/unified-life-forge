// useWeather — primary React entry point. Returns the live snapshot,
 ...
 export interface UseWeatherResult {
   ...
   /** Dynamic age of the data in minutes, computed from last_updated_unix */
 ...
 export function useWeather(
   ...
   // Update nowTick every minute for dynamic data age calculation
   useEffect(() => {
   ...
   const dataAgeMinutes = useMemo(() => {
     ...
     const lastUpdated = snapshot.meta.last_updated_unix ?? Date.now();
     return Math.max(0, Math.round((nowTick - lastUpdated) / 60_000));
   }, [snapshot, nowTick]);

   return { snapshot, status, error, tier, isRefreshing, refresh, lastResponses, dataAgeMinutes };
 }
