  ...
  if (poet.isLoading) {
    return (
      <div className="min-h-screen bg-background pt-14 px-5 flex flex-col items-center justify-center">
        ...
        <p className="text-mini text-muted-foreground font-tajawal">جاري فتح مخطوطة الشاعر…</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background text-foreground pb-page px-5 pt-14 font-tajawal selection:bg-primary/20 selection:text-primary-foreground">
      ...
      <div className="max-w-lg mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          ...
          >
            <span className="font-amiri font-bold text-hero text-primary-foreground leading-none select-none">
              {firstLetter}
            </span>
          </div>
          ...
          {p.title && (
            <div className="text-center mb-3">
              <span className="inline-block text-micro font-tajawal text-primary bg-primary/10 border border-primary/20 px-2.5 py-0.5 rounded-md whitespace-nowrap">
                {p.title}
              </span>
            </div>
          )}

          <p className="text-center text-mini text-muted-foreground font-tajawal mb-4 select-none">
          ...
          {/* فقرة سيرة كاملة */}
          {p.bio && (
            <p className="text-mini leading-[1.9] text-muted-foreground text-center max-w-md mx-auto mb-6 px-1 whitespace-pre-line">
              {p.bio}
            </p>
          )}
          ...
          <div className="grid grid-cols-3 border-y border-border/60 py-4 mb-6">
            <div className="text-center">
              <p className="font-amiri text-display font-bold text-primary leading-none mb-1 select-all">
                {p.poems_count}
              </p>
              <p className="text-micro text-muted-foreground font-tajawal select-none">
                قصائد مأثورة
              </p>
            </div>
            <div className="text-center border-e border-border/60">
              ...
              <p className="text-micro text-muted-foreground font-tajawal select-none">بيت شعر</p>
            </div>
            <div className="text-center border-e border-border/60 flex flex-col justify-center items-center">
              <p className="font-amiri text-meta font-bold text-primary leading-snug mb-1 truncate max-w-full px-1">
                {eraName || 'قديم'}
              </p>
              <p className="text-micro text-muted-foreground font-tajawal select-none">
                العصر الأدبي
        ...
        <div className="flex items-center gap-2 mb-3 px-1">
          <span className="text-micro text-primary" aria-hidden="true">
            ◆
          </span>
          <h3 className="text-mini font-bold uppercase tracking-wider text-muted-foreground font-tajawal">
            قصائده وديوانه
          </h3>
        </div>
        ...
        {poems.isLoading && page === 0 ? (
          ...
          <div className="text-center py-12 text-muted-foreground text-mini font-tajawal">
          ...
          <div className="flex flex-col">
            ...
            {showLoadMore && (
              <>
                ...
                <button
                  ...
                  className="w-full mt-4 py-3 rounded-lg bg-card border border-border text-mini font-semibold text-muted-foreground hover:text-foreground hover:border-primary active:scale-[0.98] transition disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {poems.isFetching ? (
                    <>
            ...
            {reachedEnd && items.length > PAGE && (
              <p className="text-center text-micro text-muted-foreground font-tajawal pt-6 select-none">
