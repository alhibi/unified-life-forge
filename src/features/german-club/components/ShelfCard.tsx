    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex flex-col min-w-0">
          ...
          {shelf.title_de && (
            <span
              className="text-xs font-mono font-medium text-muted-foreground tracking-wide mt-0.5"
              dir="ltr"
      ...
      <div className="flex items-center justify-between pt-3 border-t border-[hsl(var(--track))] text-xs">
        <div className="flex items-center gap-1.5 flex-wrap">
          {shelf.situation_tags.slice(0, 3).map((tag) => (
            <span key={tag} className="text-[0.625rem] font-medium text-muted-foreground bg-secondary px-2 py-0.5 rounded">
              #{tag}
            </span>
          ))}
        </div>
